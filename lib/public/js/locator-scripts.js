...
function signup_user(element) {
  ...
  posting = $.post($(this).attr('action'), $(this).serialize() );
    posting.done(function(response, textStatus, jqXHR){
      ...
      modal.find('.modal-body').text("Nach Absenden des Formulars muss dein Account durch einen Admin freigeschaltet werden.<br> Die Admins werden per Mail über den neuen Account informiert und ihn schnellstmöglich freischalten.<br> Du erhälst nach erfolgreicher Freischaltung eine Mail.");
      modal.modal();
    ...
    posting.fail(function(response, textStatus, jqXHR) {
      var modal = $('#myModal');
      modal.find('.modal-header').text("Achtung");
      modal.find('.modal-body').text("Benutzername bereits vergeben!");
      modal.modal();
      return false;
    });
    return false;
